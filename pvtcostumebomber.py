...
MAIN_OWNER = 8179218740
SUPER_ADMIN_NAME = "@SIDIKI_MUSTAFA_92"
SUPER_ADMIN_LINK = "https://t.me/SIDIKI_MUSTAFA_92"
SUPER_ADMINS = [8179218740]

BOT_TOKEN = "8740766594:AAFu54wkCaYgkwIH-g6B85z9XmojFbEtx-k"
LOG_CHANNEL_ID = --1002266975510
...
# ========== UPDATED FORCE JOIN FUNCTIONS WITH NUMBERED BUTTONS ==========

def force_join_text(missing: list) -> str:
    """Generate force join text with numbered channels"""
    lines = [
    ...
    for i, ch in enumerate(missing, 1):
        # Show channel with number and link
        channel_title = ch.get('title', f'Channel {i}')
        lines.append(f"\n{em(EMOJI_BELL, '🔔')} <b>{i}. <a href='{ch['link']}'>{channel_title}</a></b>")
    
    lines.append(f"\n\n<i>{sc('sabhi join karne ke baad verification button dabayein.')}</i>")
    return "\n".join(lines)

def force_join_kb(missing: list) -> InlineKeyboardMarkup:
    """Generate force join keyboard with numbered channel buttons and verification"""
    rows = []
    
    # First row: Channel number buttons (1,2,3,4,5)
    channel_buttons = []
    for i, ch in enumerate(missing, 1):
        channel_buttons.append(btn(f"{i}", f"fj:join:{i}", EMOJI_BELL, "🔔"))
    
    if channel_buttons:
        rows.append(channel_buttons)
    
    # Separate rows: Join links with channel numbers
    for i, ch in enumerate(missing, 1):
        channel_title = ch.get('title', f'Channel {i}')
        rows.append([btn_url(f"📢 {i}. {sc('Join')} {channel_title[:15]}", ch["link"], EMOJI_BELL, "🔗")])
    
    # Final row: Verification button
    rows.append([btn("✅ ᴠᴇʀɪғʏ & ʀᴇғʀᴇsʜ", "fj:check", EMOJI_CHECK, "✅")])
    
    return InlineKeyboardMarkup(inline_keyboard=rows)

# ========== END OF UPDATED FORCE JOIN FUNCTIONS ==========
...
async def fj_check(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.answer("✅ Verified! Sabhi channels join kar liye!", show_alert=True)
...
# ========== NEW: HANDLE INDIVIDUAL CHANNEL JOIN CLICKS ==========

@R.callback_query(F.data.startswith("fj:join:"))
async def fj_join_channel(cq: CallbackQuery, state: FSMContext):
    """Show channel join link when user clicks numbered button"""
    channel_num = cq.data.split(":")[-1]
    d = load()
    fj = d.get("force_join", {})
    channels = fj.get("channels", [])
    
    # Find the channel by number (1-indexed)
    try:
        idx = int(channel_num) - 1
        if 0 <= idx < len(channels):
            ch = channels[idx]
            await cq.answer(f"📢 {ch.get('title', 'Channel')} ka link bhej raha hoon!", show_alert=False)
            # Send the join link as a message
            await cq.message.answer(
                f"{em(EMOJI_BELL, '📢')} <b>{ch.get('title', 'Channel')}</b>\n\n"
                f"Join karein: <a href='{ch['link']}'>{ch['link']}</a>\n\n"
                f"{em(EMOJI_CHECK, '✅')} Join karne ke baad verification button dabayein.",
                parse_mode="HTML",
        ...
        await cq.answer("❌ Invalid channel number!", show_alert=True)
